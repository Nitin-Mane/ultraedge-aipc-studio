...
logger = logging.getLogger("qwen_omni")


class LegacyDynamicCache(Cache):
    ...
    def to_legacy_cache(self):
        return tuple(zip(self.key_cache, self.value_cache, strict=False))

transformers.cache_utils.DynamicCache = LegacyDynamicCache
DynamicCache = LegacyDynamicCache
...
def make_stateful(
    ...
    for kv_name_pair in zip(key_value_input_names, key_value_output_names, strict=False):
...
# Cache compiled blobs to disk: first compile serializes, later loads deserialize
# in seconds instead of recompiling (which has crashed with heap corruption on GPU).
_OV_CACHE_DIR = os.environ.get(
    "OV_CACHE_DIR",
    str(settings.OV_CACHE_DIR),
)
try:
    Path(_OV_CACHE_DIR).mkdir(parents=True, exist_ok=True)
    core.set_property({"CACHE_DIR": _OV_CACHE_DIR})
except Exception:
    pass  # caching is an optimization; never block model load on it
...
def convert_qwen2_5_omni_model(model_id, output_dir, quantization_config=None, use_local_dir=False):
    ...
    config.talker_config.pad_token_id = None
    model = Qwen2_5OmniForConditionalGeneration.from_pretrained(
        ckpt, config=config, torch_dtype=torch.float16, low_cpu_mem_usage=True
    )
    model.eval()
    processor = AutoProcessor.from_pretrained(ckpt, trust_remote_code=False)
    ...
    if not thinker_lang_path.exists():
        ...
        for input, input_name in zip(ov_model.inputs, input_names, strict=False):
            input.get_tensor().set_names({input_name})

        for output, output_name in zip(ov_model.outputs, output_names, strict=False):
...
class OVQwen2_5OmniModel(GenerationMixin):
    ...
    def __init__(
        ...
        self.config = AutoConfig.from_pretrained(model_dir, trust_remote_code=False)
        ...
        self.speaker_map = {}
        self.token2wav_solver = token2wav_solver.strip().lower()
        self.token2wav_steps = max(2, min(int(token2wav_steps), 32))
        self.last_generation_timings: dict[str, float | int | str] = {}
    ...
    def enable_talker(self, model_path, device, token2wav_device=None):
        ...
        # FP16 inference of the DIT/BigVGAN vocoder overflows on GPU and yields a
        # silent (all-zero) waveform — force f32 precision there.
        t2w_config = {"PERFORMANCE_HINT": "LATENCY"}
        if "GPU" in str(token2wav_device).upper():
            t2w_config["INFERENCE_PRECISION_HINT"] = "f32"
        self.token2wav_dit = core.compile_model(model_path / TOKEN2WAV_DIT_NAME, token2wav_device, t2w_config)
        self.token2wav_bigvgan = core.compile_model(model_path / TOKEN2WAV_BIGVGAN_NAME, token2wav_device, t2w_config)
        self.has_talker = True

    def load_speakers(self, path):
        for key, value in torch.load(path, weights_only=True).items():
            if isinstance(value, dict):
                for tensor_key in ("ref_mel", "cond"):
                    tensor = value.get(tensor_key)
                    if torch.is_tensor(tensor):
                        value[tensor_key] = tensor.detach().cpu().float().contiguous()
    ...
    def generate(
        ...
        stream_config=None,
        thinker_max_new_tokens: int = 64, # 1024
        talker_max_new_tokens: int = 128 , # 4096
        ...
        token2wav_solver: str | None = None,
        token2wav_steps: int | None = None,
        token2wav_guidance_scale: float = 0.5,
        token2wav_sway_coefficient: float | None = -1.0,
        **kwargs,
    ):
        r"""
        ...
        thinker_seconds = time.perf_counter() - stage_started

        if not generate_audio:
            self.last_generation_timings = {
                "thinker_seconds": round(thinker_seconds, 4),
        ...
        talker_seconds = time.perf_counter() - stage_started
        talker_generate_codes = talker_result[:, talker_input_ids.shape[1] : -1]

        print("[===start token2wav===]")
        token2wav_started = time.perf_counter()
        # 3. Generate wavs from code
        reference_mel_spectrogram = speaker_params["ref_mel"]
        conditioning_vector = speaker_params["cond"]
        maximum_duration = int(
            talker_generate_codes.shape[1]
            * self.config.token2wav_config.dit_config.repeats
        )
        if maximum_duration <= 0:
            raise RuntimeError("Talker generated no audio codec tokens")
        # Allocate only the duration actually requested. The previous fixed
        # 30,000-frame tensor dominated allocation time and memory for short replies.
        initial_state = torch.randn(
            [1, maximum_duration, self.config.token2wav_config.dit_config.mel_dim],
        ...
        guidance_scale = float(token2wav_guidance_scale)
        sway_coefficient = token2wav_sway_coefficient
        ...
        initial_time = 0
        solver_method = (token2wav_solver or self.token2wav_solver).strip().lower()
        ode_steps = max(2, min(int(token2wav_steps or self.token2wav_steps), 32))
        ...
        generated_waveform = ode_solver.integrate_final(time_embedding)
        dit_seconds = time.perf_counter() - token2wav_started
        ...
        bigvgan_seconds = time.perf_counter() - bigvgan_started
        waveform = waveform.squeeze().cpu().float()
        total_seconds = time.perf_counter() - total_started
        self.last_generation_timings = {
            "thinker_seconds": round(thinker_seconds, 4),
            "talker_seconds": round(talker_seconds, 4),
            "token2wav_dit_seconds": round(dit_seconds, 4),
            "token2wav_bigvgan_seconds": round(bigvgan_seconds, 4),
            "token2wav_seconds": round(time.perf_counter() - token2wav_started, 4),
            "total_seconds": round(total_seconds, 4),
            "solver": solver_method,
            "ode_steps": ode_steps,
            "dit_inferences": (ode_steps - 1) * (1 if solver_method == "euler" else 4),
            "audio_frames": maximum_duration,
            "audio_generated": True,
        }
        logger.info("Omni generation timings: %s", self.last_generation_timings)
