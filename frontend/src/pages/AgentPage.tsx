...
import { Bot, X, Settings, ChevronDown, ChevronRight } from 'lucide-react'
...
export function AgentPage({ workspacePath, onClose, settings, onSave }: {
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm' onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className='w-full max-w-lg bg-aurora-surface border border-aurora-border/40 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden' onClick={e => e.stopPropagation()}>
        ...
        <div className='p-5 space-y-4'>
          ...
          <button onClick={() => setShowAdvanced(!showAdvanced)} className='flex items-center gap-1 text-[10px] text-text-muted hover:text-text-secondary transition-colors'>
            ...
            {showAdvanced && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className='overflow-hidden'>
                <div>
                  ...
                  <textarea value={local.systemPrompt} onChange={(e) => setLocal(s => ({ ...s, systemPrompt: e.target.value }))} className='w-full h-40 px-2 py-1.5 rounded bg-aurora-base border border-aurora-border/50 text-[10px] font-mono text-text-primary resize-none focus:outline-none focus:border-qwen-violet' />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
